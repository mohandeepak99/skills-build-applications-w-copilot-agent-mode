...
function Activities() {
  ...
  const fetchData = () => {
    if (!API_ENDPOINT) {
      const msg = '[Activities] Missing REACT_APP_CODESPACE_NAME';
      console.error(msg);
      setError(msg);
      setLoading(false);
      return;
    ...
    setLoading(true);
    setError(null);
    console.log('[Activities] Fetching from endpoint:', API_ENDPOINT);
        ...
        <div className="card-header bg-white">
          <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            <div>
              ...
              <p className="text-muted mb-0">Browse activities data from the backend API.</p>
            </div>
        ...
        <div className="card-body">
          ...
          <form className="row g-2 align-items-center mb-4" onSubmit={(event) => event.preventDefault()}>
            <div className="col-auto">
              <label htmlFor="activitiesFilter" className="col-form-label fw-semibold">
                Filter
              </label>
            </div>
